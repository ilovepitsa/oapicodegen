...
- `GOLANG_SERVER_BODY_REQUEST_NO_AUTO_DEFAULTS` — когда `off`, server request-decoder
  вызывает `req.Body.SetDefaults()` для заполнения default-полей из спеки.
- `USE_REQUIRED_V2` — флаг заведён в конфиг и `ProjectFeatures`, генераторной поддержки
  `x-request-required`/`x-response-required` пока нет (T24g — pending).
...
URL-form encoding (`application/x-www-form-urlencoded`): для схем, на которые
ссылается form-urlencoded request body, генерируются методы
`MarshalURLForm() (url.Values, error)` и `UnmarshalURLForm(values url.Values) error`.
Поддерживаются примитивные поля (string/integer/number/boolean + date-time/date);
arrays/maps/`$ref`/`optional.Optional[T]`/binary → runtime-ошибка. Client
кодирует body через `MarshalURLForm()` + `values.Encode()`, server декодирует
через `ParseForm()` + `UnmarshalURLForm(PostForm)`.

`SetDefaults()`: для object-схем с `default`-полями генерируется метод
`func (m *<Name>) SetDefaults()`, заполняющий zero-value поля из спеки.
Server request-decoder вызывает его автоматически (если флаг
`GOLANG_SERVER_BODY_REQUEST_NO_AUTO_DEFAULTS` выключен).

**Не поддерживается (бэклог):**
- `x-audit-data` + `ServerAuditData` — комплаенс-логирование (audit-схема на
  каждую операцию, описывающая что логировать).
- `GenerateConverters` — автоконвертеры `Request→Response` (только при split-mode).
- `USE_REQUIRED_V2` — `x-request-required`/`x-response-required` (T24g).
- Validator TUI, Terraform-provider, TypeScript-генератор — отдельные компоненты,
  не относятся к Go-генератору.
