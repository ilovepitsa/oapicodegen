...
`-input` указывает на корневой каталог проекта. CLI обходит подпапки и
находит сервисы по наличию `<service>/src/openapi/openapi.yaml`. Каждый
сервис — отдельный `parser.Project` со своими `Model`, `Paths`, `Features`.
Все проекты собираются в `ProjectSet`; `SchemaIndex` индексирует top-level
схемы по абсолютному пути spec-файла для разрешения cross-service `$ref`.

```
Walk → Parse → ProjectSet + SchemaIndex → Generate (per-project) → CompileCheck
```

| Компонент | Файл | Ответственность |
|-----------|------|-----------------|
| `walkServices` | `internal/parser/project_loader.go` | Обход `<input>` и обнаружение `<service>/src/openapi/openapi.yaml` |
| `ProjectLoader.Load` | `internal/parser/project_loader.go` | Парсит каждый сервис, строит `ProjectSet` + `SchemaIndex` |
| `Project` | `internal/parser/project_set.go` | Один сервис: `Folder`, `SpecPath`, `Features`, `Model`, `Paths`, `OutputDir`, `ImportPrefix` |
| `ProjectSet` | `internal/parser/project_set.go` | `Common *Project` + `Projects []*Project` + `ByName map[string]*Project` |
| `SchemaIndex` | `internal/parser/schema_index.go` | `map[absPath+name]*SchemaEntry` для cross-service `$ref` |
| `markExternalRefs` | `internal/parser/source_marking.go` | Заполняет `Schema.SourceFile`/`OwnerProject`/`ExternalRef` |
| `Generate` | `internal/generator/generator.go` | Рендер Go-файлов для одного `Project` (вызывается в цикле по `ps.Projects`) |
| `CompileCheck` | `internal/codegen/compilecheck.go` | `go build ./...` в output-каталоге; требует `go.mod` |

Per-service override generation flags — файл `<service>/generation_flags.yaml`
рядом со спекой. Глобальный конфиг (`-generation-flags-config-path`) задаёт
`defaultValue`/`targetValue`/`dependsOn`; per-service override перекрывает
`defaultValue` для конкретного сервиса. Лоадер — `internal/parser/generation_flags_loader.go`.

Cross-service `$ref` (ссылка на схему из `<other-service>/src/openapi/openapi.yaml`)
разрешается через `SchemaIndex.LookupForMode(absPath, schemaName, mode)`. В
сгенерированном коде появляется алиас-импорт
`<alias> "<owner-import>/model"` и тип `<alias>.<Type>`, где `alias` —
последний компонент import-path в нижнем регистре.
...
строит промежуточную модель: `ProjectSet`, `Project`, `Schema`, `SchemaIndex`,
`Paths`, `Service`, `Method`, `PathImports`.
...
`-skip-compile-check`, `-generation-flags-config-path`, `-log-*`),
инициализирует logger и FileWriter, вызывает `parser.NewProjectLoader().Load()`
для обхода каталога и сборки `ProjectSet` + `SchemaIndex`, затем в цикле
вызывает `generator.Generate(fw, project, si)` для каждого проекта. FileWriter
оборачивается `codegen.WithPath(fw, project.Folder)`, чтобы файлы проекта
ложились в `<output>/<service>/...`. После генерации запускает
`codegen.CompileCheck(output)` (если не `-dry-run` и не `-skip-compile-check`).
