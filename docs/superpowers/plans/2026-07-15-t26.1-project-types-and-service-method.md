...
**Goal:** Rewrite the uncommitted `project_set.go`/`resources_set.go` (T26.1) with the rev2 design — `Project` aligned with `../api/apigen` (`Folder`+`Model`+`Paths`+`Features`), `SchemaIndex` (renamed from `ResourcesSet`), `PathImports` with 8 typed `gogen.Import`. Then (T26.1a) introduce `Service`/`Method` abstraction, split `Document` into `Model.schemas` + `Paths.Services`.
